...
- Rank every genuinely new comic selection only by the mean priority of its distinct exact cards: sum the priorities and divide by the number of distinct cards. The unread comic with the highest priority density wins; use comic ID only as a deterministic tie-breaker.
- Keep corpus importance/PageRank as a separate informational analysis. It must not influence scheduling, including tie-breaking. Analytics target IDs used by comic importance must never replace or alias exact SRS card IDs.
- The UI may normalize card-priority densities against the current maximum to show relative fit, but it must also identify the underlying average and exact-card count and must not present importance as part of scheduler fit. Permanently exclude every completed comic from later selection; completion is a read tombstone, not a one-step cooldown. When no unread comic remains, return and display an explicit collection-complete state instead of repeating a comic.
