...
The catalog contains 6,465 shared cards and 14,768 printed word occurrences. The word `bien` occurs 23 times across 20 comics and has seven contextual meaning cards; its ordinary “well; properly” card is shared by four comics. In *Dibujar estrellas*, the time qualification belongs to `de momento`; the `bien` card keeps its reusable meaning and a separate occurrence note explains its use. The strip's English joke-summary card has been removed: its word and grammar cards provide the language help, and normal progress reconciliation discards the removed card's history and active-session links on reload.
