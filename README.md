...
- The sidebar then shows that word's meaning card plus any expression, grammar, or contextual idea cards connected to that exact occurrence.
...
The seed curriculum contains six locally cached, manually annotated Spanish comics: 27 bubbles, 530 ordered word occurrences, 342 context-aware word-meaning cards, and 66 atomic expression, grammar, or idea cards. Repeated exact forms share a card when their meanings match; polysemous forms split by sense, while accents and conjugations remain distinct. Content integrity is validated when `lib/content.ts` loads.
